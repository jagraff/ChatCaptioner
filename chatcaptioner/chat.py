...
class AskQuestions():
    ...
    def summarize(self):
        if self.model in VALID_GPT3_MODELS:
            ...
            summary, n_tokens = call_chatgpt(summary_prompt, model=self.model, max_tokens=self.max_gpt_token)
        elif isinstance(self.model, Blip2):
            summary_prompt = prepare_gpt_prompt(
                        self.question,
                        self.questions, self.answers,
            ...
            n_tokens = 0 # local model. no token cost on OpenAI API.
            summary = self.model.call_llm(summary_prompt)
        else:
...
def summarize_chat(questions, answers, model, max_gpt_token=100, tags=[]):
    question_instruction = QUESTION_INSTRUCTION
    if len(tags) > 0:
        question_instruction = "I have an image. " \
                               "Ask me questions about the content of this image. " \
                               "Carefully ask me informative questions to maximize your information about this image content. " \
                               "Each time ask one question only without giving an answer. " \
                               f"Some (but not all) of this list of tags might be relevant to the image: '{tags}'. " \
                               "Make sure to ask some questions about the tags. " \
                               "Don't repeat any questions. " \
                               "I'll put my answer beginning with \"Answer:\"." \
...
def caption_gif(blip2, gif_path, tags, model, n_rounds=10, n_blip2_context=-1, print_mode='no',
    ...
    for frame in ImageSequence.Iterator(gif):
        new_frame = frame.convert(mode="RGB")
